...
3. `kind` 只能是：`Start` `End` `Click` `ClickText` `Wait` `TypeText` `Pause` `Manual` `LoopStart` `LoopEnd` `IfVision` `IfText` `LoopWhile`
...
| `needle` | IfText / ClickText 的目标屏幕文字 |
| `match_exact` | 文字完全相等（默认 false=包含即可） |
| `case_sensitive` | 文字是否区分大小写 |
...
- 屏幕出现某段文字才走 A → `IfText`（同样必须 true/false；字段用 `needle`）
- 识别到文字并点击其位置 → `ClickText`
...
- 不要把 Wait 的时长写成毫秒
- 不要给 IfVision / IfText 只连 true 不连 false
- 不要漏掉 LoopEnd
